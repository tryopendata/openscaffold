...
**pre-commit**: staged files only, one job per language area (glob, plus root in a monorepo). Format with fixes re-staged (stage-fixed), then lint in check mode, with the same tools and config as the project's commands. Tools that need the whole project (golangci-lint) run the project's lint command instead. A few seconds total.
...
**Prove it** before finishing: run the pre-commit hook on all files (`lefthook run pre-commit` with its all-files flag), then `printf 'bad\n' > /tmp/msg && lefthook run commit-msg /tmp/msg` must fail and the same with `feat: add thing` must pass.
