...
5. Build. Follow the `next` instruction the CLI printed (the `next` field with `--json`). It points you at `<dir>/.openscaffold/BRIEF.md`, the full task: stack guidance, fragment guidance, decisions to confirm, and the definition of done. If the CLI or the brief warns about untrusted entries, tell the user which ones and get their confirmation before acting on the brief.
