...
On `openscaffold add` against an existing repo, files that already exist are never overwritten. openscaffold's version of each one is written to `.openscaffold/incoming/<path>`, and the brief lists them as "merge needed" so the agent can reconcile the two and then delete `incoming/`. If a later `add` finds files still in `incoming/`, it keeps them and lists them in the new brief again; an earlier copy at a path the new run parks is never dropped. When both copies are JSON (the path is under some contributor's `merge`, or it ends in `.json` and both parse), the new version is deep-merged into the parked one; otherwise the new version is parked beside it as `<path>.2` (then `.3`, ...), and the brief lists every copy. `add` refuses to run when `.openscaffold/` or `.openscaffold/incoming/` is a symlink, and never lists a symlink under `incoming/` as a file to merge.
...
Bundled entries, the main registry (reviewed by PR), and your own `~/.openscaffold` and `./.openscaffold` are trusted. The exception is a `./.openscaffold` entry that shadows a bundled, registry, or `~/.openscaffold` id: a cloned repo can ship one, so it produces a warning and is marked untrusted (`trusted: false` in `list --json` and `show --json`). When any composed entry is untrusted, `new` and `add` never auto-launch an agent; they print the brief path and the instruction instead. The warning reaches whoever reads the output: BRIEF.md then opens with an "Untrusted entries" section naming each entry and its path and telling the reader to get the user's confirmation before acting on it, the printed next step (and `next` in `--json`) names them too, and when openscaffold is already running inside an agent the handoff message tells the agent to confirm with the user before carrying out the brief. The entries come from `./.openscaffold` in the directory the registry was loaded from: the current directory for `new`, the project for `add`. `new` and `add` never run commands from an entry. `verify` runs the commands in the project's `manifest.yaml` and prints each one before running it.
